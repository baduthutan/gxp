...
use App\Exports\BookingCharterByAreaReport;
...
class BookingReportCharterByAreaController extends Controller
{
    public function index(BookingReportByAreaRequest $request)
    {
        ...
        $data = [
            'page_title' => 'BOOKING REPORT CHARTER FILTERED BY AREA',
        ...
        if ($request->to_master_sub_area_id) {
            ...
            $data['sub_area'] = MasterArea::query()
                ->with(['master_sub_area' => function ($q) {
                    $q->where('is_active', '1')->whereIn('is_charter', [1, 2]);
                }])->where('is_active', '1')->whereIn('is_charter', [1, 2])->where('area_type', $area_type)->get();
        }

        return view('admin.bookingreportcharterbyarea.index', $data);
    }
    ...
    public function export_xlx(BookingReportByAreaRequest $request)
    {
        ...
        $title = "Report_booking_charter_by_area{$now}.xlsx";

        return Excel::download(new BookingCharterByAreaReport($request), $title);
    }
}
