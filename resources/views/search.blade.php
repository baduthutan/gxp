                        ...
                        <div class="card-body">
                                                ...
                                                <div class="card-body">
                                                    <div class="row">
                                                        <div class="col-sm-12 col-md-5">
                                                            ...
                                                            <span>Date Departure: {{ \Carbon\Carbon::parse($date_departure)->format('d M Y') }}</span>
                                                            @if ($booking_type == 'shuttle')
                                                                <br>
                                                                <span>Time Departure: {{ date('h:i A', strtotime($item->time_departure)) }}</span>
                                                            @endif
                                                        </div>
                                                        <div class="col-sm-12 col-md-5">
                                                            <p>
                                                                @if($booking_type == 'charter')
                                                                    Vehicle: {{ $item->vehicle_name }} (Max: {{ $item->total_seat }} passengers)
                                                                @endif
                                                                <br>Price: ${{ $item->price }}
                                                            </p>
                                                        </div>
                                                        <div class="col-sm-12 col-md-2 text-center">
                                                            <button 
                                                                ...
                                                                @if(!$item->is_available)
                                                                    onclick="show_swal('This schedule is unavailable. Please contact admin.')"
                                                                @endif
                                                            >
                                                                Choose
                                                            </button>
                                                ...
                                                <div class="card-body">
                                                    <div class="row">
                                                        <div class="col-sm-12 col-md-5">
                                                            ...
                                                            <span>Date Departure: {{ \Carbon\Carbon::parse($date_departure_2)->format('d M Y') }}</span>
                                                            @if ($booking_type == 'shuttle')
                                                                <br>
                                                                <span>Time Departure: {{ date('h:i A', strtotime($item->time_departure)) }}</span>
                                                            @endif
                            ...
                            <div class="col-sm-12 mt-4 text-center">
                                <form id="bookingForm" method="post" action="/booking">
                                    ...
                                    <input type="hidden" name="departure_schedule_id" id="departure_schedule_id" value="">
                                    ...
                                    @if($is_roundtrip)
                                        ...
                                        <input type="hidden" name="return_schedule_id" id="return_schedule_id" value="">
                                    @endif
                                    <button type="submit" class="btn btn-primary font-weight-bold btn-block"> <i class="fas fa-save"></i> Submit Booking</button>
                                </form>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        button.addEventListener('click', () => {
            ...
            if (direction === 'departure') {
                const currentChosen = document.querySelector('.choose-schedule[data-direction="departure"].btn-info');
                if (currentChosen) {
                ...
                const currentChosen = document.querySelector('.choose-schedule[data-direction="return"].btn-info');
                if (currentChosen) {
                    currentChosen.classList.remove('btn-info');
                    currentChosen.classList.add('btn-secondary');
                    currentChosen.textContent = 'Choose';
